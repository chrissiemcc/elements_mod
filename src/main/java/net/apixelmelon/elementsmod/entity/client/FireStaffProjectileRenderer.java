...
public class FireStaffProjectileRenderer extends EntityRenderer<FireStaffProjectileEntity> {
    ...
    public FireStaffProjectileRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new StaffProjectileModel(pContext.bakeLayer(ModModelLayers.STAFF_PROJECTILE_LAYER));
        this.shadowRadius = 0.5f;
    }

    public void render(FireStaffProjectileEntity entity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(FireStaffProjectileEntity pEntity) {
        return TEXTURE;
    }
}