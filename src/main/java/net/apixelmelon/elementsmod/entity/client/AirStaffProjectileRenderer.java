...
public class AirStaffProjectileRenderer extends EntityRenderer<AirStaffProjectileEntity> {
    ...
    public AirStaffProjectileRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new StaffProjectileModel(pContext.bakeLayer(ModModelLayers.STAFF_PROJECTILE_LAYER));
        this.shadowRadius = 0.5f;
    }

    public void render(AirStaffProjectileEntity entity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(AirStaffProjectileEntity pEntity) {
        return TEXTURE;
    }
}