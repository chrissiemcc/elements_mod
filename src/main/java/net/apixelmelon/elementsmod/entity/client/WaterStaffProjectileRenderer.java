...
public class WaterStaffProjectileRenderer extends EntityRenderer<WaterStaffProjectileEntity> {
    ...
    public WaterStaffProjectileRenderer(EntityRendererProvider.Context pContext) {
        super(pContext);
        model = new StaffProjectileModel(pContext.bakeLayer(ModModelLayers.STAFF_PROJECTILE_LAYER));
        this.shadowRadius = 0.5f;
    }

    public void render(WaterStaffProjectileEntity entity, float pEntityYaw, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight) {
        pPoseStack.pushPose();
        ...
        this.model.renderToBuffer(pPoseStack, vertexconsumer, pPackedLight, OverlayTexture.NO_OVERLAY, 0f, 0f, 1f, 1f);
    ...
    @Override
    public ResourceLocation getTextureLocation(WaterStaffProjectileEntity pEntity) {
        return TEXTURE;
    }
}